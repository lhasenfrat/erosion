...
public class MeshGenerator : MonoBehaviour
{
    ...
    public float frequenceIsland = 0.002f;
    // Start is called before the first frame update
    public List<MeshGenerator> lODMeshes;
    ...
    void GenerateTerrain()
    {
        ...
        for (int i = 0, z = 0; z < zSize; z++)
        {
            ...
            for (int x = 0; x < xSize-z%2; x++)
            {
                ...
                float newZ =  z * 0.866f;
                float NoiseFactor = (Mathf.PerlinNoise(frequenceIsland*(transform.position.x + newX * scale), frequenceIsland*(transform.position.z + newZ * scale))*2 -1);
                
                float newY = CustomPerlinNoiseFunction(transform.position.x + newX*scale, transform.position.z + newZ*scale)*NoiseFactor;
    ...
    {
        foreach (MeshGenerator lm in lODMeshes)
        {
            lm.Amplitude = Amplitude;
            lm.Frequency = Frequency;
            lm.UpdateTopology(); 
        }
